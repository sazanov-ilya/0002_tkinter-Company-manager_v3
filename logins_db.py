...
class LoginsDB(db.DB):
    ...
    def get_logins_list_by_id_connection_for_admin(self, id_connection, logins_filter_dict):
        ...
        match logins_filter_dict:
            ...
            case {'login_name': login_name, 'login_description': login_description, **remainder} if not remainder:
                f_login_name = ('%' + login_name.lower() + '%')
                f_login_description = ('%' + login_description.lower() + '%')
                try:
                    self.c_sqlite3.execute(
                        '''SELECT id_login,
                                  login_name,
                           ...
                           FROM logins 
                           WHERE id_connection = ?
                             AND LOWER(login_name) LIKE ?
                             AND LOWER(login_description) LIKE ?
                        ''', [id_connection, f_login_name, f_login_description]
                    )
                except sqlite3.Error as err:
                ...
                try:
                    self.c_sqlite3.execute(
                        '''SELECT id_login,
                           ...
                           WHERE id_connection = ?
                             AND LOWER(login_description) LIKE ?
                        ''', [id_connection, f_login_description]
                    )
                except sqlite3.Error as err:
                    mb.showerror("ОШИБКА!", 'get_logins_list_by_id_connection_for_admin:\n' + err.__str__())
    ...
    # def get_logins_list_by_id_connection_for_admin(self, id_connection):
    #     """ Процедура возврата списка логинов по id_connection (вывод на форму)
    #     :param id_connection:
    #     :return: Набор кортежей со списком логинов согласно фильтров
    #     """
                    ...
                    self.c_sqlite3.execute(
                        '''SELECT l.id_login,
                           ...
                           WHERE l.id_connection = ?
                             ...
                             AND ur.id_user = ?
                             AND LOWER(l.login_name) LIKE ?
                             AND LOWER(l.login_description) LIKE ?
                        ''', [id_connection, id_user, f_login_name, f_login_description]
                    )
                except sqlite3.Error as err:
                    ...
                    self.c_sqlite3.execute(
                        '''SELECT l.id_login,
                           ...
                           WHERE l.id_connection = ?
                             ...
                             AND ur.id_user = ?
                             AND LOWER(l.login_name) LIKE ?
                        ''', [id_connection, id_user, f_login_name]
                    )
                except sqlite3.Error as err:
                    mb.showerror("ОШИБКА!", 'get_logins_list_by_id_connection_for_user:\n' + err.__str__())
                    ...
                    self.c_sqlite3.execute(
                        '''SELECT l.id_login,
                           ...
                           WHERE l.id_connection = ?
                             ...
                             AND ur.id_user = ?
                             AND LOWER(l.login_description) LIKE ?
                        ''', [id_connection, id_user, f_login_description]
                    )
                except sqlite3.Error as err:
                    mb.showerror("ОШИБКА!", 'get_logins_list_by_id_connection_for_user:\n' + err.__str__())
    ...
    # def get_logins_list_by_id_connection_for_user(self, id_connection, id_user):
    #     """ Процедура возврата списка логинов по id_connection и id_user (вывод на форму)
    #     :param id_connection:
    #     :param id_user:
    #     :return: Набор кортежей со списком логинов согласно фильтров
    ...
    #     try:
    #         self.c_sqlite3.execute(
    #             '''SELECT l.id_login, l.login_name, l.login_password, l.login_description
    #                FROM logins as l,
    #                     permission_by_roles as pr,
    #                     users_roles as ur
    #                WHERE l.id_connection = ?
        ...
        try:
            self.c_sqlite3.execute(
                '''SELECT r.id_role,
                   ...
                   FROM(SELECT id_role, role_name FROM roles) as r
