...
class UserRoles(tk.Frame):
    ...
    def init_user_roles(self):
        ...
        # Кнопки
        # 1
        btn_open_filter = tk.Button(frm_top_toolbar, text='Фильтр', **self.main.btn_style,
                                    command=self.open_filter_user_role)
        btn_open_filter.pack(side=tk.LEFT, **self.main.btn_pack_padding)
        # 2
        ...
        self.btn_back_to_users.pack(side=tk.RIGHT, **self.main.btn_pack_padding)
    ...
    def open_update_user_role(self):
        ...
        if len(self.role_list_not_user) > 0:  # Если есть роли неназначенные пользователю
            if self.table.focus() != '':  # Выбран элемент в тпблице?
...
class UserRole(tk.Toplevel):
    ...
    def init_user_role(self):
        ...
        # Резировая ячейка для списка ролей
        self.rowconfigure(1, weight=1)
        self.columnconfigure(1, weight=1)

        # # 1 Метка для пользователя (1 строка, занимает 5 столбцов)
        # lbl_login = ttk.Label(self, text="Логин", width=10)
        # lbl_login.grid(row=0, column=0, columnspan=5, **lbl_padding)
