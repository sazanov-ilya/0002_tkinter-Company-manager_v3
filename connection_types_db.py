...
class ConnectionTypesDB(db.DB):
    ...
    def get_connection_type_list_by_filter(self, connection_types_filter_dict):
        ...
        match connection_types_filter_dict:
            ...
            case {'connection_type_name': connection_type_name,
                  'connection_type_description': connection_type_description}:
                f_connection_type_name = ('%' + connection_type_name.lower() + '%')
                f_connection_type_description = ('%' + connection_type_description.lower() + '%')
                try:
                    self.c_sqlite3.execute(
                        '''SELECT id_connection_type, connection_type_name, connection_type_description
                           FROM connection_types 
                           WHERE LOWER(connection_type_name) LIKE ?
                            AND LOWER(connection_type_description) LIKE ?''',
                        [f_connection_type_name, f_connection_type_description]
                    )
                except sqlite3.Error as err:
                    mb.showerror("ОШИБКА!", err.__str__())
            # connection_type_name
                ...
                try:
                    self.c_sqlite3.execute(
                        '''SELECT id_connection_type, connection_type_name, connection_type_description
                           FROM connection_types 
                           WHERE LOWER(connection_type_name) LIKE ?''',
                        [f_connection_type_name]
                    )
                except sqlite3.Error as err:
                    mb.showerror("ОШИБКА!", err.__str__())
            # connection_type_description
            case {'connection_type_description': connection_type_description}:
                f_connection_type_description = ('%' + connection_type_description.lower() + '%')
                try:
                    self.c_sqlite3.execute(
                        '''SELECT id_connection_type, connection_type_name, connection_type_description
                           FROM connection_types 
                           WHERE LOWER(connection_type_description) LIKE ?''',
                        [f_connection_type_description]
                    )
                except sqlite3.Error as err:
                    mb.showerror("ОШИБКА!", err.__str__())
            # Прочее
